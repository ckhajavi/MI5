...
{
    //TESTING for errors...
    int myCash = ui->lineEdit_TotalCash->text().toDouble();
    qDebug() << myCash;
    int stockPrice = ui->lineEditCalculatePurchase->text().toDouble();
    qDebug() << stockPrice;

    if (ui->lineEdit_TotalCash->text().toDouble() < ui->lineEditCalculatePurchase->text().toDouble()) //Not enough money to make trade.
    {
...
void LoginPage::on_btnRefresh_clicked()
{
    ...
    //The following code DOES THE DOWNLOAD and writes to StockInfo.txt.
    //We need to parse the file and update the stockList
